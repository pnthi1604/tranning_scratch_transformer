...
def torcheval_f_beta(
    ...
    return torch.tensor(
        (1 + beta ** 2) * (precision_item * recall_item) / (beta ** 2 * precision_item + recall_item + esi),
        dtype=recall.dtype
    ).to(recall.device)

def torchtext_bleu_score(
    refs: list,
    cands: list,
):
    scores = []
